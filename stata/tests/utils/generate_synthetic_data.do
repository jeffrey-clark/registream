/*==============================================================================
  Generate Synthetic Dataset for RegiStream Testing

  Purpose: Generate synthetic LISA-like data for autolabel testing
  Author: Jeffrey Clark
  Date: October 2025

  This creates a minimal synthetic dataset with realistic value ranges based on
  actual LISA summary statistics, but with completely synthetic data.

  No real data is used. All values are randomly generated for demonstration.
==============================================================================*/

* Note: This script is designed to be called from other test scripts
* It will save the synthetic dataset in stata/tests/data/

clear all
set seed 20251018  // Reproducible synthetic data
version 16.0
...
* Check if TEST_LOGS_DIR is already set (called from master test suite)
if "$TEST_LOGS_DIR" == "" {
	* Running standalone - set up paths
	local cwd = "`c(pwd)'"

	* If we're in utils/, go up one level to tests/
	if (strpos("`cwd'", "/utils") > 0) {
		local test_dir = subinstr("`cwd'", "/utils", "", .)
	}
	else {
		local test_dir = "`cwd'"
	}
	...
	* Start logging
	cap log close
	log using "$TEST_LOGS_DIR/generate_synthetic_data.log", replace text
	local standalone_log = 1
}
else {
	* Called from another script - logging already set up
	local standalone_log = 0
}

* Get script directory (works when called from parent script)
local script_dir = "`c(pwd)'"

* Set number of observations (keeping it reasonable for tests)
set obs 1000

di as text _n "Creating synthetic dataset with 1,000 observations..."
...
* Use global if set, otherwise use relative path
if "$TEST_DATA_DIR" != "" {
    local save_path "$TEST_DATA_DIR/synthetic_lisa.dta"
}
else {
    * Fallback: assume running from tests directory
    local save_path "data/synthetic_lisa.dta"
}

save "`save_path'", replace

di as text _n(2) "{hline 78}"
di as result "SUCCESS: Synthetic dataset created"
di as text "{hline 78}"
di as text "Location: `save_path'"
di as text "Observations: " as result _N
di as text "Variables: " as result r(k)
di as text _n "This synthetic dataset is ready for autolabel testing."
di as text "{hline 78}" _n

*------------------------------------------------------------------------------
* Close log if running standalone
*------------------------------------------------------------------------------
if (`standalone_log' == 1) {
	log close
}
