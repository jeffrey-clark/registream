...
program define autolabel
	...
	* Check if we have $registream_dir override
	if "$registream_dir" != "" {
		...
		_autolabel_utils confirmdir "$registream_dir"
	...
	* Otherwise proceed with default registream locations
		...
		* Use centralized OS detection from _autolabel_utils
		_autolabel_utils get_dir
	...
	* Construct the file paths
	...
	_autolabel_utils download_extract, zip("`var_filepath_zip'") zipfold("`var_filepath_zipfold'") ///
    csv("`var_filepath_csv'") dta("`var_filepath_dta'") file("`var_filename'") ///
    autolabel_dir("`autolabel_dir'") clean("variables")
    ...
    if "`label_type'" == "variables" {
		...
		preserve
			...
			* reduce number of rows for improved speed (and since we do not need distribution data for labelling)
			...
			_autolabel_utils summarize_dataset
	...
	* ----- AUTOLABEL VALUE LABELS ---------
		...
		* Construct the file paths
		...
		_autolabel_utils download_extract, zip("`val_filepath_zip'") zipfold("`val_filepath_zipfold'") ///
    csv("`val_filepath_csv'") dta("`val_filepath_dta'") file("`val_filename'") ///
    autolabel_dir("`autolabel_dir'") clean("values")
				...
				if (`is_string') {
					...
					* Store the new labels in locals
					forval k = 1(2)`nwords' {
						...
						_autolabel_utils escape_ascii "`code'"
					...
					* Replace labels with our metadata labels
					...
					file write myfile " _autolabel_utils escape_ascii " _char(96) "val" _char(39) _n
