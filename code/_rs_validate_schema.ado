...
program define _rs_validate_schema
	...
	* Check schema version characteristic
	...
	if (_rc == 0 & "`schema_ver'" != "") {
		if ("`schema_ver'" != "1.0") {
			di as error ""
			di as error "Schema version mismatch!"
			di as error "  Found: Schema `schema_ver'"
			di as error "  Required: Schema 1.0"
			di as error ""
			di as error "Your metadata file is incompatible with this version of autolabel."
			di as error "Please ensure you are using Schema 1.0 metadata files."
			di as error ""
			exit 198
		}
	}
	else {
		* No schema version found - incompatible format
		di as error ""
		di as error "No schema version found in metadata file!"
		di as error "autolabel requires Schema 1.0 metadata format."
		di as error ""
		di as error "Please ensure your metadata files include the schema_version characteristic."
		di as error ""
		exit 198
	}
		...
		* Required columns for variables schema
		...
		foreach col of local required_cols {
			...
			if (_rc != 0) {
				...
				di as error "Please check that your CSV file follows the Schema 1.0 format."
				di as error ""
				exit 198
			}
		}
